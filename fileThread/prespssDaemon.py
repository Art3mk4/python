...
class FileReader(threading.Thread):
    ...
    def run(self):
        ...
        global exitFlag
        global workQueue
        logger.info('START THREAD: id' + str(self.threadId))
        threadLock.acquire()
        if not workQueue.empty():
            ...
            if not fileName in readedFile:
                ...
                logger.info('Прочитано файлов: ' + str(readedFile))
        threadLock.release()
        logger.info('END THREAD: ' + self.name)

    def readFile(self, fileName):
        logger.info('Файл прочитан: ' + fileName + " Поток: id" + str(self.threadId))
        ...
        os.remove(fileName)
        contentFile.close()
        logger.info('Файл удалён: ' + fileName + " Поток: id" + str(self.threadId))
...
class Application():
    ...
    def threadStart(self):
        global workQueue
        fileReader = FileReader(self.threadId, workQueue)
        fileReader.start()
